...
<div class="container" >
	...
	<div id="myCarousel" class="carousel" data-ride="carousel" data-interval="false">
	  ...
	  <div class="carousel-inner">
		...
		<c:forEach items="${cm}" var="j" varStatus="vs">
		<div class ="item ${vs.index ==0?"active":"" }">
	      <div style= "width:60%; margin: auto;">
	        <div align="center"><h2> 코  인  명: ${j}</h2></div> <br/>
			<div class='left-box' align="center">
			시  가 : <fmt:formatNumber value= "${vmdata.get(j).opening_p}" pattern="#,###"/> <br/>
			종   가 : <fmt:formatNumber value= "${vmdata.get(j).closing_p}" pattern="#,###"/><br/>
			</div>
			<div class='right-box' align="center">
			최고가 :<fmt:formatNumber value= "${vmdata.get(j).max_p}" pattern="#,###"/><br/>
			최저가 : <fmt:formatNumber value= "${vmdata.get(j).min_p}" pattern="#,###"/><br/>
			</div>
			<br/>
			
			<div class='right-box' align="center" style="font-size: 15px;">
			등락률 : <fmt:formatNumber type="number"  pattern="#,###" value="${((vmdata.get(j).closing_p - vmdata.get(j).opening_p )/vmdata.get(j).opening_p)*100 }"/>%<br/> 
			</div>
			
			<div class='left-box' align="center" style="font-size: 15px;">
			등락가 : <fmt:formatNumber type="number"  pattern="#,###" value="${(vmdata.get(j).closing_p - vmdata.get(j).opening_p )}"/>
			</div>
		  </div>
		  </div>
		</c:forEach>
