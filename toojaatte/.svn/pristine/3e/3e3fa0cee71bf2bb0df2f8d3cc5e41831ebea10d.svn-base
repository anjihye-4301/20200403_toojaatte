...
$(function(){
	$("#id").on("keyup", function(){
		var id = $("#id").val().trim();
		const idCheckDiv = document.querySelector('#idCheck');
		$("#id").val(id);
		if(id.length == 0){
			...
			$.ajax({
				type:"GET",
				url:'/member/ajax/idCheck?id=' + id,
				success:function(result, status, xhr){
					idCheckDiv.innerHTML = '<p></p>';
				},
				error:function(){
					idCheckDiv.innerHTML = "<p style='color: red;'>아이디가 중복됩니다. 사용 가능한 아이디를 입력해주세요.</p>";
				}
			})
		}
	}); // 아이디 중복체크 끝
...
<body>
	<div class="container">
		...
		<h2>회원가입</h2>
		<form action="signIn.do" method="post" id="joinForm">
			<!-- 			아이디입력 -->
